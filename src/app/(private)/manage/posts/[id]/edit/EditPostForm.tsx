      ...
      <form action={formAction} className="space-y-4">
        ...
        <div>
          ...
          <TextareaAutosize
            ...
            minRows={8} onChange={handleContentChange} value={content}
          />
          {state.errors.content && (
            <p className="text-red-500 text-sm mt-1">{state.errors.content.join(',')}</p>
          )}
        </div>
        <div className="text-right textsm text-gray-500 mt-1">
          文字数： {contentLength}
        </div>
        <div>
        ...
        <RadioGroup value={published.toString()} name="published" onValueChange={(value) => setPublished(value === 'true')}>
          ...
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="false" id="publised-two" />
            <Label htmlFor="publised-two">非表示</Label>
          </div>
        </RadioGroup>
